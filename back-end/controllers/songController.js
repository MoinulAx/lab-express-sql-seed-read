...
songController.get("/:id", async (req, res) => {
    ...
    if (songById) { // Change to check if songById is not null
...
songController.post("/", async (req, res) => {
    ...
    res.status(201).json(newSong); // Set status to 201 for created resource
});

songController.delete("/:id", async (req, res) => {
    const { id } = req.params;
    const deletedSong = await deleteSong(id);
    if (deletedSong) { // Check if deletedSong is truthy
